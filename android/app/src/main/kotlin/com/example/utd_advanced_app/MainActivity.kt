...
class MainActivity: FlutterActivity() {
    ...
    override fun configureFlutterEngine(@NonNull flutterEngine: FlutterEngine) {
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, CHANNEL).setMethodCallHandler { call, result ->
            ...
            // d. Jembatan MethodChannel: Membalikkan String NIM (Tantangan Anti-AI)
            else if (call.method == "reverseNIM") {
                val nim = call.argument<String>("nim") ?: ""
                val reversed = nim.reversed()
                
                // Tampilkan Native Toast Android
                Toast.makeText(this, "NIM Asli: $nim\nNIM Dibalik (Kotlin): $reversed", Toast.LENGTH_SHORT).show()
                
                // Kembalikan hasil ke Dart
                result.success(reversed)
            }
            
            // e. Jika Dart minta fungsi yang belum kita buat
            else {
